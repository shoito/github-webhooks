...
const createOctokit = async (installationId: number): Promise<Octokit> => {
  if (!APP_ID || !PRIVATE_KEY) {
    throw new Error('GitHub App ID or private key not found');
  }
  
  return new Octokit({ authStrategy: createAppAuth, auth: { appId: APP_ID, privateKey: PRIVATE_KEY, installationId } });
};

async function getPullRequestDetails(octokit: Octokit, owner: string, repo: string, issueNumber: number) {
  try {
...
async function monitorWorkflowJobs(
  ...
  let completed = false;
  const workflowName = await getWorkflowName(octokit, owner, repo, runId);

  while (!completed) {
    const jobs = await getJobsForWorkflowRun(octokit, owner, repo, runId);

    for (const job of jobs) {
      if (job.status === 'completed') {
        await updateJobStatus(octokit, owner, repo, sha, workflowName, job);
...
functions.http('githubWebhook', async (req, res) => {
  ...
  const prDetails = await getPullRequestDetails(octokit, owner, repo, issueNumber);
  if (!prDetails) {
  ...
  try {
    ...
    const runId = await triggerWorkflow(octokit, owner, repo, workflowId, prDetails.ref, {});
    await monitorWorkflowJobs(octokit, owner, repo, runId, prDetails.sha);
  } catch (error) {
